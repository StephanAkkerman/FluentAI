...
from fluentai.services.card_gen.constants.languages import VOCAB_LANGUAGES
...
class VocabularyManager:
    def __init__(
        ...
        ----------
        lang_code : str
            The language code for the vocabulary as used by G2P (ISO 639-3).
        ...
        """
        # Map the ISO 639-3 to ISO 639-1 language code
        mapped_lang_code = map_language_code(lang_code)

        # Change the mapped language to match the supported language
        special_mapppings = {
            "zh-cn": "zh_cn",
            "zh-tw": "zh_tw",
        ...
        lang_code = special_mapppings.get(mapped_lang_code, mapped_lang_code)
        logger.debug(f"Using '{lang_code}' as the language code.")

        if lang_code not in VOCAB_LANGUAGES:
            raise ValueError(
                f"Language code '{lang_code}' is not supported. Supported languages: {VOCAB_LANGUAGES}"
            )
...
if __name__ == "__main__":
    # Initialize the VocabularyManager for Afrikaans ('afr')
    vocab_manager = VocabularyManager(lang_code="afr")
