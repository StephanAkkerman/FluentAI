...
class ALINE(_Distance):
    ...
    def alignments(self, src, tar, score_only=False):
        ...
        def _sig_sub(seg1, seg2):
            return self._c_sub - _delta(seg1, seg2) - _sig_vwl(seg1) - _sig_vwl(seg2)
        ...
        def _retrieve(i, j, score, out):
            ...
            if s_mat[i, j] == 0:
                ...
                if (
                    i > 0
                    and j > 0
                    and s_mat[i - 1, j - 1] + _sig_sub(src[i - 1], tar[j - 1]) + score
                    >= threshold
                ):
                ...
                if (
                    j > 0
                    and s_mat[i, j - 1] + _sig_skip(tar[j - 1]) + score >= threshold
                ):
                    loc_out = deepcopy(out)
                ...
                if (
                    i > 0
                    and s_mat[i - 1, j] + _sig_skip(src[i - 1]) + score >= threshold
                ):
                    loc_out = deepcopy(out)
        ...
        for i in range(1, src_len + 1):
            for j in range(1, tar_len + 1):
                s_mat[i, j] = max(
                    ...
                    s_mat[i - 1, j - 2] + _sig_exp(src[i - 1], tar[j - 2], tar[j - 1])
                    if j > 1
                    else NINF,
                    s_mat[i - 2, j - 1] + _sig_exp(tar[j - 1], src[i - 2], src[i - 1])
                    if i > 1
                    else NINF,
