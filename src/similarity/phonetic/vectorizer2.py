...
def vectorize_in_parallel(token_ipa_list, max_workers=8):
    # Pre-allocate a list to store results at the correct index
    vectors = [None] * len(
        token_ipa_list
    )  # This ensures the correct order is maintained
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        ...
        for idx, word in tqdm(
            ...
            futures[executor.submit(vectorize_word, word)] = idx

        # As the tasks complete, store the result in the correct position
        for future in tqdm(
            as_completed(futures), total=len(futures), desc="Vectorizing words"
        ):
            idx = futures[future]  # Retrieve the index associated with this future
            vectors[idx] = future.result()  # Place the result in the correct position
